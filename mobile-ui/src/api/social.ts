...
export const getFollowers = (username: string): Promise<SocialProfile[]> => {
  return axiosInstance
    .get<SocialProfile[]>(`/profiles/${username}/followers`)
    .then((response) => response.data);
};

export const getFollowing = (username: string): Promise<SocialProfile[]> => {
  return axiosInstance
    .get<SocialProfile[]>(`/profiles/${username}/following`)
    .then((response) => response.data);
};
...
export const getBlockedUsers = (): Promise<BlockedUser[]> => {
  return axiosInstance
    .get<BlockedUser[]>('/profiles/blocked-users')
    .then((response) => response.data);
};
...
export const checkBlockStatus = (
  ...
  return axiosInstance
    .get<BlockStatus>(`/profiles/check-block/${username}`)
    .then((response) => response.data);
};
