...
export const getUsers = (params: UserFilterParams) => {
  return axiosInstance.get<PaginatedResponse<AdminUserSummary>>(
    '/admin/users',
    { params },
  );
};
...
export const getListsForUser = (userId: number) => {
  return axiosInstance.get<AdminUserListSummary[]>(
    `/admin/users/${userId}/lists`,
  );
};

export const getReviewsForUser = (userId: number) => {
  return axiosInstance.get<AdminReviewSummary[]>(
...
export const getReportsMadeByUser = (userId: number) => {
  return axiosInstance.get<AdminUserReportSummary[]>(
    `/admin/users/${userId}/reports-made`,
  );
};
