...
export const createList = (data: UserListForCreation): Promise<UserList> => {
  return axiosInstance
    .post<UserList>('/user-lists', data)
    .then((response) => response.data);
};

export const updateList = (
  listId: number,
  data: UserListForUpdate,
): Promise<void> => {
  return axiosInstance
    .put(`/user-lists/${listId}`, data)
    .then((response) => response.data);
};
...
export const getMyListDetail = (listId: number): Promise<UserListDetail> => {
  return axiosInstance
    .get<UserListDetail>(`/user-lists/${listId}/my-detail`)
    .then((response) => response.data);
};
...
export const addGameToList = (
  ...
  return axiosInstance
    .post(`/user-lists/${listId}/games`, { gameId })
    .then((response) => response.data);
};

export const removeGameFromList = (
  listId: number,
  gameId: number,
): Promise<void> => {
  return axiosInstance
    .delete(`/user-lists/${listId}/games/${gameId}`)
    .then((response) => response.data);
};
...
export const getFollowedListsByMe = (
  ...
  return axiosInstance
    .get<PaginatedResponse<UserListPublic>>('/user-lists/followed-by-me', {
      params,
    })
    .then((response) => response.data);
};
...
export const checkWishlistStatus = (gameId: number) => {
  return axiosInstance
    .get<{ isInWishlist: boolean }>(`/user-lists/wishlist/${gameId}/status`)
    .then((res) => res.data);
};
...
export const getFavoritesList = async (
  ...
  const response = await axiosInstance.get<UserList | null>(
    `/user-lists/user/${username}/favorites`,
  );
  return response.data;
};
