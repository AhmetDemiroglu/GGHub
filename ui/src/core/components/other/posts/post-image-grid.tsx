...
export function PostImageGrid({ images, className }: PostImageGridProps) {
    ...
    const cell = (image: PostImage, extra?: string) => (
        <div key={image.url} className={cn("relative overflow-hidden bg-muted", extra)}>
            <Image
    ...
    if (sorted.length === 1) {
        return (
            <div className={cn("mt-3 overflow-hidden rounded-xl border border-border/50", className)}>
                {cell(sorted[0], "aspect-[16/10] w-full")}
            </div>
        );
    }
    ...
    if (sorted.length === 2) {
        return (
            <div className={cn("mt-3 grid grid-cols-2 gap-0.5 overflow-hidden rounded-xl border border-border/50", className)}>
                {sorted.map((image) => cell(image, "aspect-square"))}
            </div>
        );
    }
    ...
    if (sorted.length === 3) {
        return (
            <div className={cn("mt-3 grid grid-cols-2 gap-0.5 overflow-hidden rounded-xl border border-border/50", className)}>
                {cell(sorted[0], "row-span-2 aspect-[1/2]")}
                {cell(sorted[1], "aspect-square")}
                {cell(sorted[2], "aspect-square")}
            </div>
        );
    }
