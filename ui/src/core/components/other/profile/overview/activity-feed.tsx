...
import { Star, List as ListIcon, UserPlus, ArrowDown } from "lucide-react";
...
export default function ActivityFeed({ activities }: ActivityFeedProps) {
    ...
    if (!activities || activities.length === 0) {
        return <div className="text-center py-10 text-muted-foreground text-sm">{t("profile.activityFeed.noActivity")}</div>;
    }
    ...
    return (
        <div className="relative space-y-6 pb-0">
            {visibleActivities.map((activity) => (
                <div key={`${activity.type}-${activity.id}`} className="relative pl-6 border-l border-border/50 last:border-0 pb-0 last:pb-0 group">
                    <div className="absolute -left-2.5 top-0 h-5 w-5 rounded-full border-4 border-background bg-muted-foreground/30 group-hover:bg-primary group-hover:scale-110 transition-all" />
                    <div className="flex flex-col gap-2">
                        <div className="text-sm text-muted-foreground">
                            <span className="text-xs font-medium bg-muted px-1.5 py-0.5 rounded text-foreground mr-2">{dayjs(activity.occurredAt).fromNow()}</span>

                            {activity.type === ActivityType.Review && activity.reviewData && (
                                <span>
                                    <span className="font-semibold text-foreground">{activity.reviewData.game.name}</span> {t("profile.activityFeed.reviewText")}
                                </span>
                            )}
                            {activity.type === ActivityType.ListCreated && activity.listData && (
                                <span>
                                    <span className="font-semibold text-foreground">{activity.listData.name}</span> {t("profile.activityFeed.listText")}
                                </span>
                            )}
                            {activity.type === ActivityType.FollowUser && activity.followData && (
                                <span>
                                    <span className="font-semibold text-foreground">@{activity.followData.username}</span> {t("profile.activityFeed.followText")}
                                </span>
                            )}
                        </div>
                        ...
                        {activity.type === ActivityType.Review && activity.reviewData && (
                            <Link
                                href={buildLocalizedPathname(`/games/${activity.reviewData.game.slug}#review-${activity.reviewData.reviewId}`, locale)}
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-sm text-foreground/80 line-clamp-2 italic">"{activity.reviewData.contentSnippet}"</p>
                                </div>
                            </Link>
                        )}
                        ...
                        {activity.type === ActivityType.ListCreated && activity.listData && (
                            <Link
                                href={buildLocalizedPathname(`/lists/${activity.listData.listId}`, locale)}
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        {activity.listData.gameCount} {t("profile.activityFeed.gamesLabel")}
                                    </p>
                                </div>
                            </Link>
                        )}
                        ...
                        {activity.type === ActivityType.FollowUser && activity.followData && (
                            <Link
                                href={buildLocalizedPathname(`/profiles/${activity.followData.username}`, locale)}
                ...
                <>
                    ...
                    <div className="relative z-20 flex flex-col items-center gap-3 pt-4 -mt-16 pb-2">
                        <span className="text-xs text-muted-foreground font-medium">
                            {t("profile.activityFeed.moreLabel", { count: activities.length - visibleCount })}
                        </span>
                        <button
                            onClick={() => setVisibleCount((prev) => prev + 5)}
                            className="group relative cursor-pointer outline-none"
                            aria-label={t("profile.activityFeed.moreAria")}
                        >
