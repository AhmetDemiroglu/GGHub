        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-4">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-xl font-bold tracking-tight">{t("homeBento.trendingTitle")}</h3>
                </div>
                        ...
                        <Link href={`/games/${game.slug || game.rawgId}`} key={game.id} className="group">
                            <Card className="h-full border-border/50 bg-card/50 backdrop-blur-sm overflow-hidden hover:border-primary/50 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                                ...
                                <div className="p-3">
                                    ...
                                    <p className="text-xs text-muted-foreground mt-1">{t("homeBento.popularLabel")}</p>
                                </div>
                            </Card>
                        </Link>
                    ))}
            ...
            <div className="lg:col-span-1 space-y-4">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-xl font-bold tracking-tight">{t("homeBento.leaderboardTitle")}</h3>
                </div>

                <Card className="border-border/50 bg-card/50 backdrop-blur-sm h-fit">
                    <CardHeader className="pb-0">
                        <CardTitle className="text-sm font-medium text-muted-foreground">{t("homeBento.leaderboardSubtitle")}</CardTitle>
                    </CardHeader>
                                        ...
                                        <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors group cursor-pointer">
                                            <div
                                                className={`w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold ${
                                                    index === 0
                                                        ? "bg-yellow-500 text-black shadow-yellow-500/20 shadow-lg"
                                                        : index === 1
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center justify-between mt-0.5">
                                                    <p className="text-xs text-muted-foreground truncate">{user.levelName}</p>
                                                    <span className="text-xs font-medium text-primary">{user.xp.toLocaleString()} XP</span>
                                                </div>
                                            </div>
                                        </div>
                                    </Link>
                ...
                <div className="rounded-xl bg-linear-to-r from-indigo-500 to-purple-600 p-4 text-white shadow-lg mt-4">
                    <p className="font-bold text-sm">{t("homeBento.ctaTitle")}</p>
                    <p className="text-xs text-white/80 mt-1">{t("homeBento.ctaDescription")}</p>
                </div>
            </div>
        </div>
    );
