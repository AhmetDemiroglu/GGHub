...
export function ThemeToggleButton() {
    ...
    React.useEffect(() => {
        ...
        if (isInitialMount.current) {
            // First mount: set state immediately without animation
        ...
        // Only animate if the actual theme changed (user clicked), not on re-renders/route changes
        if (prevTheme.current === resolvedTheme) {
            return;
        }

        prevTheme.current = resolvedTheme;
        // Delay to bypass next-theme's `disableTransitionOnChange` global CSS blocker
