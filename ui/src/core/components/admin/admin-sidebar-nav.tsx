...
interface AdminSidebarNavProps {
    ...
    onLinkClick?: () => void; // Mobilde 'Sheet'i kapatmak için
}
    ...
    return (
        <div className="flex h-full flex-col">
            <div className="mb-6 flex flex-col gap-6">
                <Link href="/" className="flex items-center justify-center gap-2.5 w-full cursor-pointer group" onClick={onLinkClick}>
            ...
            <nav className="flex flex-1 flex-col gap-2">
                {navItems.map((item) => (
                    <Link key={item.href} href={item.href} className={cn(buttonVariants({ variant: pathname === item.href ? "secondary" : "ghost" }), "justify-start gap-2")} onClick={onLinkClick}>
            ...
            <hr className="my-4" />
            <Link href="/" className={cn(buttonVariants({ variant: "outline" }), "justify-start gap-2")} onClick={onLinkClick}>
